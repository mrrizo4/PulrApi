...
public class CreateStoreCommandValidator : AbstractValidator<CreateStoreCommand>
{
    ...
    {
        var x = await _dbContext.Stores.AllAsync(b => b.Name.Trim().ToLower() != name.Trim().ToLower() && b.IsActive,
            ct);
        return x;
    }
    ...
    private async Task<bool> UniqueStoreNameExists(string uniqueName, CancellationToken ct)
    {
        ...
        var x = await _dbContext.Stores.AllAsync(
    ...
    {
        var x = await _dbContext.Stores.AllAsync(
            b => b.StoreEmail.Trim().ToLower() != email.Trim().ToLower() && b.IsActive, ct);
        return x;
    }
}